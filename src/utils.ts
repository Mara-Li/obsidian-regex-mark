...
export function substituteString(str: string, match: RegExpMatchArray | RegExpExecArray) {
	...
	return str.replace(substituteRegex, (fullM, $prefix: string, identifier: string = "", name?: string) => {
		...
		if (!identifier || $prefix.length % 2 === 1) return $half + "$" + identifier;
		else if (identifier === "&") return $half + match[0];
		else if (name) return $half + (match.groups?.[name] ?? "$" + identifier);
		else return $half + (match[Number(identifier)] ?? "$" + identifier);
	});
}
